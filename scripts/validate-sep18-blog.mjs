...
const source=fs.readFileSync('app/article-blog-2026-09-18.ts','utf8');
...
for(const entry of manifest.entries){
  ...
  assert.ok(html.includes('2026-09-18'),`${entry.slug}: date missing`);
  assert.ok(html.includes('datePublished'),`${entry.slug}: structured date missing`);
  assert.ok(html.includes('canonical'),`${entry.slug}: canonical missing`);
...
console.log('PASS: 12 September 18 Blog routes, 900+ words, dates, canonicals, image, and sitemap entries');
