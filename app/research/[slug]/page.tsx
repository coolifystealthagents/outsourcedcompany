...
export default async function ResearchPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Header /><main className="section"><article className="container guide-article">
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Article', headline: p.title, datePublished: (p as typeof p & { datePublished?: string }).datePublished ?? p.updated, dateModified: p.updated, mainEntityOfPage: `https://outsourcedcompany.com/research/${p.slug}` }) }} />
    <p className="eyebrow">Philippines staffing research · Updated <time dateTime={(p as typeof p & { datePublished?: string }).datePublished ?? p.updated}>{rich.displayDate ?? new Date(`${((p as typeof p & { datePublished?: string }).datePublished ?? p.updated)}T00:00:00Z`).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric', timeZone: 'UTC' })}</time></p><h1>{p.title}</h1><p className="lead">{p.excerpt}</p>
